...
export type QnaSort = 'latest' | 'popular' | 'comments' | 'views'
export type QnaListStatus = 'unresolved' | 'all' | 'waiting' | 'answered' | 'resolved'
...
export type QnaComment = { id: string; ownerUserId?: string; parentCommentId?: string; author: string; authorAvatarUrl?: string; body: string; createdAt: string; mine: boolean; isAccepted?: boolean; liked?: boolean; likes?: number; hospitalSnapshot?: HospitalSnapshot; hospital_snapshot?: HospitalSnapshot | null; payload?: { hospitalSnapshot?: HospitalSnapshot | null; hospital_snapshot?: HospitalSnapshot | null; parentCommentId?: string } }
...
export type QnaPost = { id: string; ownerUserId?: string; category: QnaCategory; categories?: QnaCategory[]; status?: QnaStatus; selectedAnswerCommentId?: string; title: string; body: string; author: string; authorAvatarUrl?: string; mine?: boolean; animal: string; petId: string; animalGroup?: string; animalSpecies?: string; image?: string; images?: string[]; linkedRecordId?: string; attachedRecordSnapshot?: AttachedRecordSnapshot; attachedDiarySnapshot?: AttachedDiarySnapshot; createdAt: string; viewCount?: number; liked: boolean; likedBy?: string[]; likes: number; comments: QnaComment[] }
