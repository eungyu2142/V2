...
export function AppNavigation(props: AppNavigationProps) {
  ...
  return <>
    <header className="fixed inset-x-0 top-0 z-40 flex h-[var(--app-header-height)] items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface)] px-8 max-[700px]:px-4">
      ...
      <button className="grid size-10 place-items-center rounded-full text-[var(--color-primary-900)] hover:bg-[var(--color-primary-50)] aria-pressed:bg-[var(--color-primary-600)] aria-pressed:text-[var(--color-on-primary)]" type="button" aria-label={activeTab === 'profile' ? '프로필 닫기' : '프로필 열기'} aria-pressed={activeTab === 'profile'} onClick={onToggleProfile}>
        {profile.avatarUrl ? <img className="size-full rounded-full object-cover" src={profile.avatarUrl} alt="" /> : <svg className="size-6 fill-none stroke-current" strokeWidth="1.6" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="4" /><path d="M4 21v-2a8 8 0 0 1 16 0v2" /></svg>}
      </button>
    </header>
