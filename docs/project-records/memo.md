...
## 2026-07-30 다이어리 높이와 모바일 날짜 상세 개선

### 요청 요약

- 데스크톱 다이어리에서 계속 보이는 바깥 스크롤을 제거한다.
- 모바일 캘린더의 날짜 상세 안에서 루틴과 상황별 기록을 처리한다.

### 분석·판단 이유

- `html`의 강제 스크롤과 캘린더 셀의 고정 높이가 합쳐져 화면보다 콘텐츠가 커졌다.
- 날짜 상세에서 별도의 새 저장 방식을 만들면 완료 상태와 DB 기록이 어긋날 수 있어 기존 컴포넌트와 콜백을 재사용했다.
...
- 데스크톱 일반 다이어리는 뷰포트 안에서 헤더와 NOTICE를 제외한 남은 높이를 캘린더와 오른쪽 패널이 사용한다.
- 오른쪽 할 일이 많을 때는 전체 페이지 대신 해당 패널 내부에서만 스크롤한다.
- 모바일 날짜 상세에는 선택 날짜 기준의 루틴 완료·취소·건너뛰기와 상황별 기록 버튼을 추가했다.
- 기존 Smart Add, 먹이 선택, 온습도 입력 모달이 날짜 상세에서도 정상 렌더링되도록 공통 오버레이 위치를 정리했다.

### 검증 결과

- `npm run lint` 통과.
- `npm run build` 통과.
- Vite의 기존 번들 크기 경고 외 오류 없음.

### 남은 작업

- 로그인된 모바일 실기기에서 키보드 및 safe-area 동작을 최종 확인한다.

## 2026-07-30 오늘 할 일 루틴 목록 버튼

### 요청 요약

- `오늘 할 일`을 버튼으로 만들고 해당 날짜의 루틴 목록을 표시한다.

### 분석·판단 이유

- 기존 목록 데이터와 완료 콜백은 이미 있으므로 새 데이터 구조를 만들지 않고 노출 방식만 접기·펼치기로 변경했다.
...
- 기본적으로 접힌 `오늘 할 일` 버튼을 추가했다.
- 버튼에는 날짜, 루틴 개수, 펼침 상태를 표시한다.
- 펼치면 밀린 할 일과 오늘 할 일 목록이 나타난다.
- 루틴 추가 버튼과 각 루틴의 체크 및 점 3개 메뉴는 기존대로 유지한다.
...
## 2026-07-30 완료한 루틴도 목록에 표시

### 요청 요약

- 오늘 할 일 버튼을 펼치면 완료한 루틴도 볼 수 있게 한다.

### 분석·판단 이유

- `daily_tasks`의 완료 상태와 기존 반복 루틴의 완료 시각을 그대로 사용해 별도 데이터 복사 없이 표시했다.
...
- 미완료 루틴과 완료 루틴을 분리했다.
- 완료 루틴에는 체크 표시, 완료 시각, `완료한 루틴` 문구를 표시한다.
- 완료 체크를 다시 누르는 기존 완료 취소 기능을 유지한다.
...
## 2026-07-30 먹이 기록 UUID 저장 오류 수정

### 요청 요약

- 먹이 루틴 완료 시 Supabase 저장 오류가 발생하는 문제를 수정한다.

### 분석·판단 이유

- `care_records.id`는 UUID 컬럼이다.
- 기존 코드는 일일 루틴 기록에 `task-${dailyTask.id}`를 사용해 UUID 형식 제약을 위반했다.
- 온습도 루틴도 같은 ID 생성 방식을 사용하고 있어 함께 수정해야 했다.
...
- 먹이 및 온습도 기록 ID를 `crypto.randomUUID()`로 생성한다.
- `saveDailyTaskCareRecord`를 추가해 같은 `daily_task_id`의 기존 기록 ID를 재사용한다.
- 재시도 시 고유 인덱스 충돌이나 중복 기록이 발생하지 않게 했다.
- 기록 저장이 성공한 후에만 일일 루틴 완료 상태를 확정한다.
...
- 실제 Supabase 사용자 세션에서 먹이 저장을 한 차례 확인한다.
## 2026-07-30 진료 기록과 병원 리뷰 상호 연결

### 요청 요약

- 진료 기록은 병원 리뷰가 없어도 독립적으로 작성한다.
- 리뷰와 진료 기록은 같은 질문 정보를 공유하고 서로 내용을 불러올 수 있다.
- 한 번 연결에 사용한 리뷰 또는 진료 기록은 다시 선택할 수 없다.

### 분석·판단 이유

- 리뷰는 공개 경험 공유이고 진료 기록은 개인 관리 데이터이므로 생성 시점을 강제로 묶지 않았다.
- 두 데이터의 고유 ID를 유지하고 `reviewId`, `clinicRecordId`로 관계만 연결해야 수정과 과거 기록 보존이 안전하다.
...
- 진료 상세 스냅샷 타입에 병원명, 방문일, 비용, 진단, 처치, 진료 경험, 다음 방문, 약 정보를 추가했다.
- 다이어리에서 새 진료 기록을 직접 작성하거나 미연결 리뷰를 선택해 내용을 채울 수 있다.
- 지도 리뷰 작성에서 현재 펫의 미연결 진료 기록을 선택해 같은 항목을 자동 입력할 수 있다.
- 리뷰만 작성하면 진료 기록을 자동 생성하지 않는다. 진료 기록을 선택한 경우에만 두 데이터를 연결한다.
- 연결된 리뷰와 진료 기록은 각각 선택 목록에서 비활성화한다.
- 다음 방문과 약 복용 일정 생성 로직은 기존 Supabase 저장 흐름을 재사용한다.

### 검증 결과

- `npm run lint` 통과.
- `npm run build` 통과.
- 로컬 로그인 화면 로딩 및 브라우저 콘솔 오류 없음 확인.

### 남은 작업

- 로그인된 실제 계정에서 리뷰→진료, 진료→리뷰 저장 후 Supabase payload와 선택 비활성화를 직접 확인한다.
